...
def add_ribbon(builder, centerline, blend_func):
    ...
    for i,(x,y) in enumerate(centerline):
        if i==0:
            dx=centerline[1][0]-x; dy=centerline[1][1]-y
        elif i==n-1:
            dx=x-centerline[i-1][0]; dy=y-centerline[i-1][1]
        else:
...
def build_turn(left=False):
    ...
    for k in range(1,17):
        t=k/16.0
        th=math.pi + (math.pi/2-math.pi)*t
...
def build_junction(cross=False):
    ...
    ARM=12.0*FT
    for dx,dy in dirs:
        pts=[(dx*s,dy*s) for s in (HALF_TOTAL, HALF_TOTAL+ARM*0.45, HALF_TOTAL+ARM)]
        def one_end(i,p):
            if i==0: return 1.0
            if i==1: return 0.55
            return 0.0
...
bad=[(i,face_nz(f)) for i,f in enumerate(builder.faces) if face_nz(f) <= 1e-10]
if bad:
    raise RuntimeError(f'{ASSET}: downward/degenerate visible faces detected: {bad[:8]}')
print(f'[TPG QA] {ASSET}: {len(builder.faces)} visible faces all upward-wound')
...
mat=bpy.data.materials.new('TPG_Ground109_Dirt_PBR')
...
ri.filepath_raw=str(rm); ri.file_format='PNG'; ri.save()
...
# Separate closed collision shell from the exact corrected top footprint.
...
road['ROAD_KIND']=KIND
road['LENGTH_FT']=LENGTH_FT if KIND=='straight' else 0
