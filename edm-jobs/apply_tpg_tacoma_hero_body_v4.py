...
# silhouette sculpt, and replaces the crude rectangular camper shell generated by
# build_tpg_tacoma.py with the photo-matched Tacoma shell.
...
# V6 front/cab silhouette correction. The FBX payload is the hero body and remains the
# source of truth; this is deliberately a small vertex-space sculpt rather than a
# procedural replacement. It attacks the two clay-QA problems left after V5: a soft,
# bulbous upper cab and an overly rounded front clip. Lower body, wheel arches, doors,
# bed and all animation objects are untouched.
body = bpy.data.objects.get("FBX_Plane.001")
if body is None or body.type != 'MESH':
    raise RuntimeError("Missing source-derived Tacoma hero body FBX_Plane.001")

cab_count = hood_count = nose_count = 0
for vert in body.data.vertices:
    x, y, z = vert.co.x, vert.co.y, vert.co.z

    # Tighten the upper double-cab greenhouse without changing the beltline. This keeps
    # the DCLB cab long but removes the inflated/van-like side profile seen in clay QA.
    if -0.82 <= x <= 1.05 and z >= 1.34:
        t = min(1.0, max(0.0, (z - 1.34) / 0.48))
        vert.co.y *= (1.0 - 0.045 * t)
        cab_count += 1

    # Give the hood/front fenders a flatter Tacoma shoulder: upper front-clip vertices
    # are drawn inward slightly toward the centerline, strongest near the hood top.
    if 1.00 <= x <= 2.58 and 1.00 <= z <= 1.43:
        tz = min(1.0, max(0.0, (z - 1.00) / 0.43))
        vert.co.y *= (1.0 - 0.035 * tz)
        # Reduce the source mesh's rounded hood crown while retaining a subtle center rise.
        center = max(0.0, 1.0 - min(1.0, abs(y) / 0.90))
        vert.co.z -= 0.018 * tz * (1.0 - 0.35 * center)
        hood_count += 1

    # Sharpen the leading nose very conservatively. This makes the front read more like
    # the squared 2012-2015 Tacoma fascia without moving lamps, bumper or wheel openings.
...
body.data.update()
print(f"[TPG TACOMA HERO V6] source sculpt cab={cab_count} hood={hood_count} nose={nose_count}")
...
print("[TPG TACOMA HERO V6] source wheel centers corrected; cab/front-clip silhouette tightened; V5 ARE-style topper retained")
